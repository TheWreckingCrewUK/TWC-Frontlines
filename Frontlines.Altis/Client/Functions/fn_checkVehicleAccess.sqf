...
if (_vehicle isKindOf "Tank" && {!(_vehicle isKindOf "APC_Tracked_02_base_F")}) exitWith {
	if ("Tank" in _roleDescription || {"Vehicle" in _roleDescription || {"Logistics" in _roleDescription}}) exitWith {
		true;
	};
	["You must be in Tank Crew, Vehicle Crew or Logistics to use this vehicle."] call AW_fnc_notify;
	if (_moveOut) then {moveOut player};
	false;
};
